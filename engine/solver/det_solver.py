...
from .det_engine import train_one_epoch, train_one_epoch_semi, evaluate
...
class SemiDetSolver(DetSolver):
    """Semi-supervised detection solver.

    Uses train_one_epoch_semi() which splits each batch into GT (supervised loss)
    and pseudo (unsupervised loss with confidence filtering).

    Config YAML should include:
        semi:
            unsup_weight: 1.0    # λ weight for unsupervised loss

    Dataset config (SemiCocoDetection) handles confidence filtering:
        conf_threshold: 0.6  # confidence threshold for pseudo labels
    """
    ...
    def fit(self, ):
        ...
        # Read semi-supervised params from config
        semi_cfg = args.yaml_cfg.get('semi', {})
        unsup_weight = semi_cfg.get('unsup_weight', 1.0)

        print(f"     ### Semi-supervised: unsup_weight={unsup_weight} ###")

        n_parameters, model_stats = stats(self.cfg)
        print(model_stats)
        print("-" * 42 + "Start semi-supervised training" + "-" * 43)
        ...
        if args.lrsheduler is not None:
            ...
            self.lr_scheduler = FlatCosineLRScheduler(
                ...
                total_epochs=args.epoches, warmup_iter=args.warmup_iter,
                flat_epochs=args.flat_epoch, no_aug_epochs=args.no_aug_epoch
            )
            self.self_lr_scheduler = True
